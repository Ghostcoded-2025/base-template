...
**Shapes**: Tables/columns/enums → **`src/types/supabase.ts`** only; regenerate with `npm run db:types`; do not hand-edit. **Do not infer** shapes from migrations. **Reasoning order**: `supabase.ts` → this file → migrations (**history only**).
